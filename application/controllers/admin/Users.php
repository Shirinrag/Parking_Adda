...
class Users extends MY_Controller {
    ...
    public function datatable_json() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $data[] = array(++$i, $row['username'], $row['email'], $row['mobile_no'], $row['car_number'], date_time($row['created_at']), '<span class="btn btn-success">' . $verify . '</span>', '<input class="tgl_checkbox tgl-ios" 
				...
				' . $status . '><label for="cb_' . $row['id'] . '"></label>', '<a title="View" class="view btn btn-sm btn-info" href="' . base_url('admin/users/edit/' . $row['id']) . '"> <i class="fa fa-eye"></i></a>
    ...
    public function add() {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array('username' => $this->input->post('username'), 'firstname' => $this->input->post('firstname'), 'lastname' => $this->input->post('lastname'), 'email' => $this->input->post('email'), 'mobile_no' => $this->input->post('mobile_no'), 'address' => $this->input->post('address'), 'password' => password_hash($this->input->post('password'), PASSWORD_BCRYPT), 'created_at' => date('Y-m-d : h:m:s'), 'updated_at' => date('Y-m-d : h:m:s'),);
