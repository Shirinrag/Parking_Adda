...
class Parking extends MY_Controller {
    ...
    public function parking_datatable() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $data[] = array(++$i, $placeinfo, $row['country_name'], $row['vendor_name'], $booking_type, $row['no_of_slots'], '<input class="tgl_checkbox tgl-ios" 
				...
				' . $status . '><label for="cb_' . $row['id'] . '"></label>', '<a title="Edit" class="update btn btn-sm btn-warning" href="' . base_url('admin/parking/editPlaces/' . $row['id']) . '"> <i class="fa fa-pencil-square-o"></i></a>
    ...
    public function addplaces() {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array('vendor_id' => $this->input->post('vendor_id'), 'placename' => $this->input->post('place_name'), 'place_address' => $this->input->post('place_address'), 'applied_slots' => $this->input->post('slots_counts'), 'fk_country_id' => $this->input->post('country_id'), 'fk_state_id' => $this->input->post('state_id'), 'city_id' => $this->input->post('city_id'), 'pincode' => $this->input->post('pincode'),);
    ...
    public function legal_datatable_json() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $place_details = "<b>Place Name:- </b>" . $row["placename"] . "<br>" . "<b>Place Address:- </b>" . $row["place_address"] . "<br>" . "<b>Country:- </b>" . $row["country_name"] . "<br>" . "<b>State:- </b>" . $row["state_name"] . "<br>" . "<b>City:- </b>" . $row["city_name"] . "<br>" . "<b>Pincode:- </b>" . $row["pincode"] . "<br>";
            $data[] = array(++$i, $place_details, $row['vendor_name'], $row['no_of_slots'], $view,);
        }
        $records['data'] = $data;
        echo json_encode($records);
    }
