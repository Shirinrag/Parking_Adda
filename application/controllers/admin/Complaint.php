...
class Complaint extends MY_Controller {
    ...
    public function datatable_json() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $data[] = array(++$i, $row['unique_booking_id'], $row['placename'], $row['place_address'], $row['complaint_text'], date_time($row['booking_from_date']), date_time($row['booking_to_date']), $row['from_time'], $row['to_time'], $row['slot_name'], $row['display_id'], $row['verifier_name'], $status, $view);
        }
        $records['data'] = $data;
    ...
    public function cc_datatable_json() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $data[] = array(++$i, '<a title="View History" href=' . base_url("admin/complaint/view_complaint/" . $row['booking_id']) . ' >' . $row['unique_booking_id'] . '</a>', $place_name, $row['complaint_text'], $date_stack1, $date_stack2, $row['verifier_name'], $source, $view,);
        }
        $records['data'] = $data;
        echo json_encode($records);
    }
    ...
    public function cc_actions($id = "") {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array('fk_despostion_id' => $this->input->post('despositions'), 'customercareRemark' => $this->input->post('cc_remarks'), 'status' => 1);
    ...
    public function getAllOtherComplaints() {
        ...
        foreach ($otherComplaints as $row) {
            ...
            $data[] = array(++$i, $row['username'], $row['complaint_topic'], $row['description'], $source, $row['created_date'], $status, $view,);
        }
        $records['data'] = $data;
    ...
    public function registerBookingComp() {
        if ($this->input->post('submit')) {
            ...
            $complaint_array = array('verifier_id' => $_POST['verifier_id'], 'place_id' => $booking_data['place_id'], 'complaint_source' => '2', 'slot_id' => $booking_data['slot_id'], 'booking_id' => $_POST['booking_id'], 'complaint_id' => $_POST['complaint_id'], 'enforcer_id' => '0', 'status' => $status, 'fk_despostion_id' => $_POST['verifier_action'], 'customercareRemark' => $_POST['cc_remark']);
    ...
    public function registerOtherComp() {
        if (!empty($_POST)) {
            ...
            $data = array('fk_tbl_complaint_id' => $otherComplaintId, 'user_id' => $_POST['user_id'], 'source_type' => $source_type, 'complaint_type_id' => $_POST['other_complaint_type'], 'fk_disposition_id' => $_POST['dispostion_id'], 'problem_description' => $_POST['problem_description'], 'cc_remark' => $_POST['cc_remark']);
    ...
    function getBlockedSlots() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $data[] = array(++$i, $place_info, $row['display_id'], date("d-m-Y H:i a", strtotime($row['issue_raised_on'])), $source, $var,);
        }
        $records['data'] = $data;
        echo json_encode($records);
    }
    ...
    function getClosedSlotsComplaints() {
        ...
        foreach ($getClosedSlotsComplaints as $row) {
            ...
            $data[] = array(++$i, $place_name, $row['display_id'], $issue_raised_on, $issue_resolved_on, $source, '',);
        }
        $records['data'] = $data;
        echo json_encode($records);
    }
