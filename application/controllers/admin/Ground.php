...
class Ground extends MY_Controller {
    ...
    public function ground_datatable_json() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $place_details = "<b>Place Address :-</b>" . $row['place_address'] . "<br>" . "<b>Country :-</b>" . $row['country_name'] . "<br>" . "<b>State :-</b>" . $row['state_name'] . "<br>" . "<b>City :-</b>" . $row['city_name'] . "<br>" . "<b>Pincode :-</b>" . $row['pincode'] . "<br>";
            $data[] = array(++$i, $row['placename'], $place_details, $row['vendor_name'], $row['no_of_slots'], $view,);
        }
        $records['data'] = $data;
    ...
    public function genrateSlots($id) {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array('no_of_slots' => $this->input->post('slots_counts'), 'latitude' => $this->input->post('latitude'), 'longitude' => $this->input->post('longitude'));
                ...
                if ($result) {
                    ...
                    for ($i = 0;$i < $slots_count;$i++) {
                        ...
                        $display_id = "P-" . ($i + 1);
                        $data = array('place_id' => $id, 'slot_name' => $slot_name, 'display_id' => $display_id, 'machine_id' => 0, 'latitude' => $this->input->post('latitude'), 'longitude' => $this->input->post('longitude'), 'onOff_apply_date' => '', 'reserved_userId' => '', 'reserved_booking_time' => '');
    ...
    public function add_device_id() {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array('device_id' => $this->input->post('device_id'));
                $data = $this->security->xss_clean($data);
    ...
    public function datatable_json() {
        ...
        foreach ($records['data'] as $row) {
            ...
            $data[] = array(++$i, $row['placename'], $row['slot_name'], $row['display_id'], $row['device_id'], $timestamp, $device_statuss, $final_status,);
        }
        $records['data'] = $data;
        echo json_encode($records);
    }
