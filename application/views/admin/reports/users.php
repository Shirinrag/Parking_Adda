          ...
          <div class="row">
            ...
            <div class="col-md-2">
              ...
              <div class="form-group">
                ...
                <input type="date" name="from_date" id="from_date" class="form-control" value="<?= date('Y-m-01') ?>" placeholder="" max="<?= date('Y-m-d') ?>" onchange="filter_data()" />
            ...
            <div class="col-md-2">
              ...
              <div class="form-group">
                ...
                <input type="date" name="to_date" id="to_date" class="form-control" value="<?= date('Y-m-d') ?>" placeholder="" max="<?= date('Y-m-d') ?>" onchange="filter_data()" />
...
<script>
  ...
  function filter_data() {
    ...
    $('#midpageloader').show();
     ...
     $.post('<?= base_url('admin/Reports/getUsersDatas') ?>', $('.filterdata').serialize(), function(data) {
      ...
      html += '<th>Car Number</th>';
      html += '<th>Device Type</th>';
      html += '<th>Registartion Date</th>';
      ...
      if (data.summary.length != '0') {
        ...
        for (var i = 0; i < data.summary.length; i++) {
            ...
            if(data.summary[i]['car_number']=='null'){
                var car_number = "No Data";
            }else{
             var car_number = data.summary[i]['car_number'];
            }
            
            if(data.summary[i]['device_type']==1){
                var device = "Android";
            }else if(data.summary[i]['device_type']==2){
                var device = "IOS";
            }else{
                var device = "Other";
            }
          ...
          html += '<td>' + data.summary[i]['fullname'] + '</td>';

          html += '<td>' + data.summary[i]['email'] + '</td>';

          html += '<td>' + data.summary[i]['mobile_no'] + '</td>';
          
          html += '<td><center>' + car_number + '</center></td>';
          
          html += '<td>' + device + '</td>';

          html += '<td>' + data.summary[i]['reg'] + '</td>';
...
<script type="text/javascript">
  ...
  function downloadCSVFile(csv_data) {
    ...
    temp_link.download = "User_Downloads.csv";
