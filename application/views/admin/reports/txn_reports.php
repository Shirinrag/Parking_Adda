            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <center><span class=""><b>Username</b></span></center>

                        <div class="form-group">

                            <select name="userid" class="form-control select2" onchange="filter_data()" >

                                <option value="all"><?= "All" ?></option>

                                <?php foreach($users as $datas):?>

                                    <option value="<?=$datas['id']?>"><?=$datas['fullname']." || ".$datas['mobile_no'] ?></option>
...
$.post('<?=base_url('admin/Reports/getTransactionsData')?>',$('.filterdata').serialize(),function(data){
    ...
    if(data.TxnInfo.length!='0'){

    for (var i = 0; i < data.TxnInfo.length; i++) {

          html +='<tr>';

              html +='<td>'+data.TxnInfo[i]['id']+'</td>';

              html +='<td>'+data.TxnInfo[i]['userid']+'</td>';

              html +='<td>'+data.TxnInfo[i]['username']+'</td>';

              html +='<td>'+data.TxnInfo[i]['contact']+'</td>';

              html +='<td>'+data.TxnInfo[i]['amount']+'</td>';

              html +='<td>'+data.TxnInfo[i]['on_created']+'</td>';

              html +='<td>'+data.TxnInfo[i]['order_id']+'</td>';

              html +='<td>'+data.TxnInfo[i]['payment_id']+'</td>';
...
<script type="text/javascript">
        ...
        function downloadCSVFile(csv_data) {
            ...
            temp_link.download = "TxnData.csv";
