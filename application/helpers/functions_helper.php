<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    if (!function_exists('get_sidebar_menu')) {
        function get_sidebar_menu()
        {
            $ci =& get_instance();
            $ci->load->database();   
            $admin_type = $ci->session->userdata('admin_role_id');
            $sql = "SELECT md.* FROM `module` as md 
                    INNER join module_access as ma on md.module_id = ma.fk_module_id
                    WHERE md.is_deleted=1 AND ma.admin_role_id=$admin_type GROUP BY 1 ORDER BY md.sort_order ASC"; 
